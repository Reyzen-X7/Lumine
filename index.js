...
global.reyzzn = new WAConnection()
...
reyzzn.logger.level = 'silent'
reyzzn.version = [2, 2142, 12]
reyzzn.browserDescription = [`${ownerName}`,'Desktop','3.0']

reyzzn.on("qr", (qr) => { console.log('\x1b[32mqr { Scan }\x1b[0m')})
fs.existsSync(session) && reyzzn.loadAuthInfo(session)
	
reyzzn.on("connecting", () => {console.log('\x1b[32mconnecting { Menghubungkan }\x1b[0m')})
reyzzn.on("open", () => {console.log('\x1b[32mopen { Terhubung }\x1b[0m')})
reyzzn.on("ws-close", () => {console.log('\x1b[31mws-close { Menghubungkan Kembali }\x1b[0m')})
reyzzn.on("close", () => {console.log('\x1b[31mclose { Terputus }\x1b[0m')})
 
await reyzzn.connect({timeoutMs: 30*1000})
fs.writeFileSync(session, JSON.stringify(reyzzn.base64EncodedAuthInfo(), null, '\t'))
try { 
pporang = await reyzzn.getProfilePicture(`${reyzzn.user.jid.split('@')[0]}@s.whatsapp.net`)
...
reyzzn.sendMessage(`${ownerNumber}@s.whatsapp.net`, `${JSON.stringify(reyzzn.user, null, 2)}`, MessageType.text, { contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title:`${botName} connected`,thumbnail:ppu,previewType:"PHOTO"}}})
link = `https://chat.whatsapp.com/FMAW2cyZkXJAK16BUyBgRA`
reyzzn.query({ json:["action", "invite", `${link.replace('https://chat.whatsapp.com/','')}`]})
ip = await fetchJson(`http://ip-api.com/json/?`, {method: 'get'})

reyzzn.on("chat-update", (mek) => {
...
copyright = `\`\`\`© by Reyzen ${d.toLocaleDateString(locale, { year: 'numeric' })}\`\`\``
...
require("./reyzn.js")(reyzzn, mek)})}
starts('./data/session.json')
